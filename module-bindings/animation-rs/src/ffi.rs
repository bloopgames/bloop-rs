...
use engine::{dst::vec::BackingStore, prelude::*};

use super::*;

static mut SPRITE_ANIMATIONS_CID: Option<ComponentId> = None;
...
pub static mut SPRITE_ANIMATIONS_SET_ANIMATION_TAG: Option<
    unsafe extern "C" fn(
        ...
        tag_bytes_ptr: *const u8,
        tag_bytes_len: usize,
    ) -> Option<NonZero<u32>>,
> = None;

static mut SPRITE_ANIMATOR_CID: Option<ComponentId> = None;
...
pub unsafe fn load_module_proc_addrs(
    ...
    unsafe {
        SPRITE_ANIMATIONS_LOAD_ASEPRITE_ANIMATION = transmute(get_proc_addr(
            c"animation::SpriteAnimations::load_aseprite_animation".as_ptr(),
            ctx,
        ));
        SPRITE_ANIMATIONS_LOAD_CUSTOM_ANIMATION = transmute(get_proc_addr(
            c"animation::SpriteAnimations::load_custom_animation".as_ptr(),
            ctx,
        ));
        SPRITE_ANIMATIONS_GET = transmute(get_proc_addr(
            c"animation::SpriteAnimations::get".as_ptr(),
            ctx,
        ));
        SPRITE_ANIMATIONS_SET_ANIMATION_TAG = transmute(get_proc_addr(
            c"animation::SpriteAnimations::set_animation_tag".as_ptr(),
            ctx,
        ));
        SPRITE_ANIMATION_NEW = transmute(get_proc_addr(
            c"animation::SpriteAnimation::new".as_ptr(),
            ctx,
        ));
        SPRITE_ANIMATION_PLAY = transmute(get_proc_addr(
            c"animation::SpriteAnimation::play".as_ptr(),
            ctx,
        ));
        SPRITE_ANIMATION_PAUSE = transmute(get_proc_addr(
            c"animation::SpriteAnimation::pause".as_ptr(),
            ctx,
        ));
        SPRITE_ANIMATION_SET_ACTIVE_ANIMATION = transmute(get_proc_addr(
            c"animation::SpriteAnimation::set_active_animation".as_ptr(),
            ctx,
        ));
    }
}
